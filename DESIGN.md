...
- [ ] **하드코딩 색상 → 상수** — 60곳+ #4A403A 등 → TEXT_HEADING 등 (시각 변화 없음, 유지보수용)
- [ ] **하드코딩 shadow → 상수** — 25곳 inline shadow → SHADOW_CARD (시각 변화 없음)
- [ ] **접근성** — aria-label 보강 (네비바, 모달 트리거 등)
- [ ] **반응형** — 태블릿/데스크톱 레이아웃 확인
- [ ] **다크 모드** — 미적용
- [ ] **빈 상태 비주얼** — SVG 일러스트 (empty state 개선)
