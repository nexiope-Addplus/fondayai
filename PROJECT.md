...
## 5-0. 최근 주요 업데이트 (2026-03-25 세션 4)

### 관리자 대시보드 전면 강화
- **신규 섹션 6개**: 실시간 접속, 리텐션(7일 재방문율), 시간대별 패턴, 이탈 지점, 화장품 카테고리 분포, 푸시 효과
- **로그인 방식 분포**: 카카오/Google/LINE 바 차트
- **접속 지역 TOP15**: Cloudflare `cf.city` 기반 자동 추출
- **유입 경로 분석**: UTM/referrer 자동 감지 (인스타/검색/직접방문)
- **화장품 현황**: 유저별 접기/펼치기 목록 (제품명/브랜드/카테고리/시간대)
- **스캔 날짜 필터**: 날짜 + 로그인/비로그인 + 로그인방식 3중 필터
- **스캔 상세보기**: 카드형 접기/펼치기 (유저ID, 피부나이, 10개 항목 점수, 기기 정보 전부)
- **비로그인 유저 식별**: IP+UA 해시 → `visitor_id` (IP 미저장, 개인정보 안전)

### 추적 인프라
- D1 scans 테이블에 `city`, `country`, `referrer`, `provider`, `device_info` 컬럼 추가
- `device_info`: deviceType(mobile/desktop), os(iOS/Android/Windows), browser(Chrome/Safari)
- App.tsx에서 UTM/referrer 자동 감지 → localStorage → 스캔 시 서버 전달
- 관리자 페이지 로드 시 D1 컬럼 자동 마이그레이션
- 스캔 저장 INSERT fallback: 새 컬럼 없는 환경에서도 기본 컬럼으로 저장 보장

### 버그 수정
- 스캔 중복 저장 방지: `challenge-token`에서 로그인 유저 D1 INSERT 스킵
- 스캔 D1 저장 실패: INSERT fallback으로 어떤 환경에서든 저장 보장

---

## 5-0-prev3. 이전 업데이트 (2026-03-24 세션 3)
...
- [x] **관리자 대시보드 강화**: 리텐션/시간대/이탈/도시/유입/기기 추적 + 상세보기 ✅
- [ ] **관리자 CSV 다운로드**: 스캔/화장품/일기 데이터 내보내기
- [ ] **개인정보처리방침 업데이트**: 도시/유입경로/기기 수집 항목 추가
- [ ] **ResultScreen 리팩토링**: 1100줄+ → 레이아웃/상태훅/섹션 컴포넌트 분리
- [ ] **포인트 활용처**: 출석 포인트 → 기능 언락 (상세 리포트 등)
...
## 14. 최근 커밋 히스토리 (2026-03-25)

| 커밋 | 내용 |
|------|------|
| b912598 | fix: 스캔 D1 저장 fallback 추가 — 새 컬럼 없어도 저장 보장 |
| 30b7ae8 | fix: 관리자 페이지 로드 시 D1 컬럼 자동 마이그레이션 |
| 7b9d997 | fix: 관리자 최근 스캔 쿼리 fallback 강화 |
| e3941fe | feat: 관리자 대시보드 — 리텐션/시간대/이탈/등급/푸시효과/실시간 |
| 6eabbde | feat: 관리자 스캔 상세보기 + 비로그인 visitor_id + 기기 추적 |
| 5c9dffc | feat: 관리자 화장품 현황 — 유저별 접기/펼치기 |
| 1362303 | fix: 스캔 중복 저장 방지 + 관리자 필터 추가 |
| 0c9072b | feat: 관리자 대시보드 + 추적 인프라 대폭 강화 |
