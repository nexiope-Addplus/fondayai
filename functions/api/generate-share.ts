...
async function getFonts(): Promise<Uint8Array[]> {
  ...
  const [r, b, bk] = await Promise.all([
    fetch(`${BASE}/Pretendard-Regular.otf`).then(x => x.arrayBuffer()),
    fetch(`${BASE}/Pretendard-Bold.otf`).then(x => x.arrayBuffer()),
    fetch(`${BASE}/Pretendard-Black.otf`).then(x => x.arrayBuffer()),
  ]);
  fontBuffers = [new Uint8Array(r), new Uint8Array(b), new Uint8Array(bk)];
  return fontBuffers;
}

// ── SVG → PNG ─────────────────────────────────────────────────────────────────
async function svgToPng(svgStr: string, fonts: Uint8Array[]): Promise<string> {
...
function wrap(str: string, maxCh: number): string[] {
  ...
  for (const w of words) {
    ...
    if (candidate.length > maxCh) {
      ...
      // if word itself too long, split by char
      let rem = w;
...
function tx(
  ...
  return `<text x="${x}" y="${y}" font-family="Pretendard" font-size="${size}" font-weight="${weight}" fill="${fill}" text-anchor="${anchor}" opacity="${opacity}">${esc(content)}</text>`;
}
...
function rectStroke(x: number, y: number, w: number, h: number, fill: string, stroke: string, rx = 0): string {
  return `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${fill}" stroke="${stroke}" stroke-width="2" ${rx ? `rx="${rx}"` : ""}/>`;
}

// Common header (returns SVG fragment + new y offset)
function header(dateStr: string): { svg: string; y: number } {
  let s = "";
  // Gradient top bar
  s += rect(0, 0, W, 10, `url(#grad)`);
  // Logo
  s += tx(PAD, 80, "FondayAI", 38, "#1E293B", { weight: 900 });
  // Date
  s += tx(W - PAD, 80, dateStr, 28, SCAN_TO, { weight: 800, anchor: "end" });
  // Divider
  s += rect(PAD, 100, W - PAD * 2, 2, "#E2E8F0");
  return { svg: s, y: 130 };
}

// Common footer (SVG fragment)
function footer(): string {
  const FY = H - 110;
  let s = rect(PAD, FY, W - PAD * 2, 80, "white", 40);
  s += tx(W / 2, FY + 52, "피부 챌린지 같이 할 사람? 검색 > Fonday AI", 26, "#1E293B", { weight: 900, anchor: "middle" });
  return s;
}

// Gradient defs (shared across slides)
const DEFS = `
...
function buildSvgSlide1(d: SlideData): string {
  ...
  const { svg: hdrSvg, y: hY } = header(dateStr);
  let s = hdrSvg;
  let y = hY + 20;
  ...
  if (rankingPercentile !== undefined) {
    ...
    s += tx(W / 2, y + 42, `TOP ${rankingPercentile}% 랭커`, 30, "#FBBF24", { weight: 900, anchor: "middle" });
    y += 84;
  }

  // Baumann type
  s += tx(W / 2, y + 180, finalType, 200, SCAN_TO, { weight: 900, anchor: "middle" });
  y += 220;
  ...
  for (const letter of letters) {
    ...
    s += tx(px + pW / 2, y + 38, baumannNames[letter] || letter, 24, c, { weight: 800, anchor: "middle" });
    px += pW + 16;
  }
  y += 80;
  ...
  s += tx(boxX + boxW / 2, y + 76, `${overallScore}${scoreSuffix}`, 72, SCAN_TO, { weight: 900, anchor: "middle" });
  s += tx(boxX + boxW / 2, y + 112, "종합 스코어", 26, "#94A3B8", { weight: 700, anchor: "middle" });

  if (hasSkinAge) {
    const bx2 = boxX + boxW + 20;
    s += rect(bx2, y, boxW, boxH, "white", 24);
    s += tx(bx2 + boxW / 2, y + 76, `${skinAge}세`, 72, "#8B5CF6", { weight: 900, anchor: "middle" });
    s += tx(bx2 + boxW / 2, y + 112, "피부 나이", 26, "#94A3B8", { weight: 700, anchor: "middle" });
  }
  y += boxH + 30;
  ...
  if (aiComment) {
    ...
    lines.forEach((ln, i) => {
      s += tx(W / 2, y + 50 + i * 50, ln, 30, "#334155", { weight: 700, anchor: "middle" });
    });
    y += cH + 16;
  }
...
function buildSvgSlide2(d: SlideData): string {
  ...
  // Title
  s += tx(PAD, hY + 50, "SKIN SPECS", 52, "#1E293B", { weight: 900 });
  s += tx(PAD, hY + 90, "10가지 세부 스펙 분석", 28, "#64748B", { weight: 700 });
  ...
  scores.forEach((sc, i) => {
    ...
    // Label
    s += tx(barX, midY + 10, label, 26, "#475569", { weight: 800 });
    // Progress bar bg
    s += rect(progressX, midY - 8, progressW, 16, "#F1F5F9", 8);
    // Progress bar fill
    s += rect(progressX, midY - 8, Math.round(progressW * sc.score / 100), 16, color, 8);
    // Score
    s += tx(barX + barW - scoreW, midY + 10, String(sc.score), 30, color, { weight: 900, anchor: "end" });
...
function buildSvgSlide3(d: SlideData): string {
  ...
  s += tx(PAD, hY + 50, "AI SOLUTION", 52, "#1E293B", { weight: 900 });
  s += tx(PAD, hY + 90, "나만을 위한 맞춤 스킨케어 처방", 28, "#64748B", { weight: 700 });
  ...
  improvements.slice(0, 3).forEach((imp, i) => {
    ...
    s += tx(PAD + 30, y + 40, imp.title, 32, "#1E293B", { weight: 900 });
    descLines.forEach((ln, j) => {
      s += tx(PAD + 30, y + 72 + j * 44, ln, 26, "#475569", { weight: 600 });
    });
    y += cardH + 20;
  });
  ...
  if (cosmetics.length > 0) {
    ...
    s += tx(PAD + 20, y + 36, "RECOMMENDED INGREDIENT", 20, "#64748B", { weight: 900 });
    s += tx(PAD + 20, y + 68, cos.key, 32, SCAN_TO, { weight: 900 });
    cLines.forEach((ln, j) => {
      s += tx(PAD + 20, y + 68 + 38 + j * 40, ln, 24, "#475569", { weight: 600 });
    });
  }
...
function buildSvgSlide4(d: SlideData): string {
  ...
  s += tx(PAD, hY + 50, "INNER BEAUTY", 52, "#1E293B", { weight: 900 });
  s += `${tx(PAD, hY + 90, finalType, 28, SCAN_TO, { weight: 900 })}<text font-family="Pretendard" font-size="28" font-weight="700" fill="#64748B"> 맞춤 이너뷰티 솔루션</text>`;
  ...
  letters.slice(0, 4).forEach((letter, i) => {
    ...
    s += tx(cx + 50, cy + 62, letter, 36, color, { weight: 900, anchor: "middle" });
    // Name
    s += tx(cx + 20, cy + 110, nut.name, 32, color, { weight: 900 });
    // Foods
    s += tx(cx + 20, cy + 148, nut.foods, 22, "#64748B", { weight: 800 });
    // Why
    whyLines.forEach((ln, j) => {
      s += tx(cx + 20, cy + 182 + j * 38, ln, 22, "#475569", { weight: 600 });
    });
  });
...
function buildSvgSlide5(d: SlideData): string {
  ...
  s += tx(PAD, hY + 50, "DANGER ZONE", 52, "#1E293B", { weight: 900 });
  s += tx(PAD, hY + 90, "오늘은 이거 딱 참아보자!", 28, "#64748B", { weight: 700 });
  ...
  function avoidSection(
    ...
    inner += tx(PAD + 20, startY + 48, label, 28, topColor, { weight: 900 });
    inner += rect(PAD + 20, startY + 60, W - PAD * 2 - 40, 2, `${topColor}40`);

    let iy = startY + 72;
    items.slice(0, 3).forEach((item, idx) => {
      const whyLines = wrap(item.why, 36);
      inner += tx(PAD + 14, iy + 32, "X", 26, "#EF4444", { weight: 900 });
      inner += tx(PAD + 46, iy + 36, item.food, 30, "#1E293B", { weight: 900 });
      whyLines.forEach((ln, j) => {
        inner += tx(PAD + 46, iy + 68 + j * 36, ln, 24, "#64748B", { weight: 600 });
      });
      iy += itemHeights[idx];
    });
...
export const onRequest = async (context: any) => {
  ...
  for (let i = 0; i < builders.length; i++) {
    try {
      ...
      const png = await svgToPng(svgStr, fonts);
      slides.push(png);
    } catch (e: any) {
