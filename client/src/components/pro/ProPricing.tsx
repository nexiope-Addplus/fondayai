...
export default function ProPricing() {
  return (
    <section id="pricing" className="py-20 bg-[#FFF8F1]">
      <div className="max-w-6xl mx-auto px-6 text-center">
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-6 items-start">
          {PLANS.map((plan) => (
            ...
            >
              {plan.badge && (
                <span className="absolute -top-3 left-6 bg-[#C9815E] text-white text-xs font-bold px-3 py-1 rounded-full">
                  {plan.badge}
                </span>
              )}
              ...
              <ul className="mt-6 space-y-3">
                {plan.features.map((feat) => (
                  <li key={feat} className="flex items-start gap-2 text-sm text-slate-600">
                    <Check size={16} className="text-[#315F72] mt-0.5 flex-shrink-0" />
                    {feat}
                  </li>
                ))}
              </ul>
              ...
              <a
                ...
                className={`mt-8 block text-center font-medium py-3 rounded-xl transition-colors ${
                  plan.highlighted
                    ? "bg-[#315F72] text-white hover:bg-[#274B5A]"
