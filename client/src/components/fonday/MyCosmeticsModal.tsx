...
import { inferCosmeticTimeOfDay, buildRoutineGuide, buildRepresentativeRoutine, buildCosmeticCorrelationSignals, apiBase } from "./utils";
...
export function MyCosmeticsModal({ onClose, onAddNew, scans = [] }: { onClose: () => void; onAddNew: () => void; scans?: any[] }) {
  ...
  useEffect(() => {
    fetch(`${apiBase()}/api/cosmetics`).then(r => r.json()).then(data => {
