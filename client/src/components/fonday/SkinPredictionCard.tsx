...
import { DEEP_GREEN, SCAN_TO, BORDER_COLOR, FONT_DISPLAY, TEXT_TERTIARY } from "./constants";
...
export function SkinPredictionCard({
  ...
  return (
    <Card className="rounded-3xl overflow-hidden bg-white/95" style={{ border: `1px solid ${BORDER_COLOR}` }}>
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold" style={{ color: DEEP_GREEN }}>{t("result.prediction.title")}</p>
        ...
        <div className="grid gap-2.5">
          <div className="rounded-2xl p-4" style={{ background: "#F0FDF4", border: "1px solid #BBF7D0" }}>
            <div className="flex items-start justify-between gap-3 mb-3">
              <div className="min-w-0">
                <p className="text-xs font-semibold text-emerald-600" style={{ fontFamily: FONT_DISPLAY }}>{t("result.prediction.bestRoute")}</p>
            ...
            <p className="text-xs font-semibold text-emerald-700 mb-2">{t("result.prediction.routine")}</p>
          ...
          <div className="rounded-2xl p-4" style={{ background: "#FFF7ED", border: "1px solid #FED7AA" }}>
            <div className="flex items-start justify-between gap-3 mb-3">
              <div className="min-w-0">
                <p className="text-xs font-semibold text-orange-500" style={{ fontFamily: FONT_DISPLAY }}>{t("result.prediction.riskRoute")}</p>
            ...
            <p className="text-xs font-semibold text-orange-700 mb-2">{t("result.prediction.risks")}</p>
            <div className="flex flex-wrap gap-1.5">
              {bad.risks?.map((risk, index) => (
                <span key={index} className="rounded-full bg-white/80 px-2.5 py-1 text-xs font-semibold text-stone-600 text-kr-pretty">
                  {risk}
                </span>
              ))}
            </div>
