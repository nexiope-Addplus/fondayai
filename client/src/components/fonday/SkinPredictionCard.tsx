            ...
            <div className="space-y-3">
              {good.routine?.map((routine, index) => (
                <div key={index} className="flex items-center gap-2 rounded-xl bg-white/70 px-3 py-2">
                  ...
                  <span className="text-[11px] font-medium text-stone-700 leading-tight text-kr-pretty">{routine}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="rounded-2xl p-4" style={{ background: "#FFF7ED", border: "1px solid #FED7AA" }}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {bad.risks?.map((risk, index) => (
                <span key={index} className="rounded-full bg-white/80 px-2.5 py-1 text-[10px] font-bold text-stone-600 text-kr-pretty">
                  {risk}
                </span>
              ))}
            </div>
