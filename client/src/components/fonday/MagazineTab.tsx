...
function DistributionBar({ label, count, pct, isMyRange }: { label: string; count: number; pct: number; isMyRange: boolean }) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] font-bold text-stone-400 w-11 shrink-0 text-right">{label}</span>
      <div className="flex-1 h-[14px] rounded-full overflow-hidden relative" style={{ background: "#F0EDE8" }}>
        ...
        />
        <span className="absolute right-2 inset-y-0 flex items-center text-[9px] font-bold text-stone-400">
          {count}
        </span>
      </div>
      {isMyRange && (
        <span className="text-[10px] font-black shrink-0" style={{ color: SCAN_TO }}>◀</span>
      )}
    ...
    <>
      <ScrollArea className="h-[calc(100dvh-60px)]">
        <motion.div className="pb-28 overflow-x-hidden" variants={stagger} initial="initial" animate="animate">
              ...
              <div className="rounded-3xl p-4" style={{ background: `linear-gradient(135deg, ${DEEP_GREEN}, #4A7C6E)`, boxShadow: "0 8px 24px rgba(45,95,79,0.22)" }}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-[44px] font-black text-white leading-none">{latestOverall}</p>
                    {avgDelta !== 0 && (
                  ...
                  <div className="shrink-0 text-right">
                    <div className="px-3 py-2 rounded-2xl mb-2" style={{ background: "rgba(255,255,255,0.15)" }}>
                      <p className="text-[10px] text-white/60 mb-0.5">{t("ranking.topLabel")}</p>
          ...
          {rankingData && (
            <motion.div variants={fadeChild} className="mx-5 mb-4">
              <div className="rounded-2xl px-4 py-3 flex items-center gap-0 bg-white" style={{ boxShadow: "0 2px 8px rgba(45,95,79,0.06)" }}>
                <div className="flex-1 text-center min-w-0">
                  <p className="text-[10px] text-stone-400 truncate">{t("ranking.avgScore")}</p>
                  <p className="text-[17px] font-black" style={{ color: DEEP_GREEN }}>{rankingData.avgScore || "—"}</p>
                </div>
                <div className="w-px h-7 bg-stone-100 shrink-0" />
                <div className="flex-1 text-center min-w-0">
                  <p className="text-[10px] text-stone-400 truncate">{t("ranking.topScore")}</p>
                  <p className="text-[17px] font-black" style={{ color: SCAN_TO }}>{rankingData.topScore || "—"}</p>
                </div>
                <div className="w-px h-7 bg-stone-100 shrink-0" />
                <div className="flex-1 text-center min-w-0">
                  <p className="text-[10px] text-stone-400 truncate">{t("result.totalScans")}</p>
                  <p className="text-[17px] font-black text-stone-700">{rankingData.totalScans || "—"}</p>
                </div>
              </div>
            </motion.div>
          )}
          ...
          {distributionBands.length > 0 && (
            <motion.div variants={fadeChild} className="mx-5 mb-4">
              <Card className="rounded-2xl overflow-hidden border-none" style={{ boxShadow: "0 2px 8px rgba(45,95,79,0.06)" }}>
                    ...
                    {weakestScores.map((s: any, i) => {
                      ...
                      return (
                        <div key={i} className="flex-1 rounded-2xl p-2.5 text-center" style={{ background: "#FFF5F5" }}>
                          <p className="text-[10px] text-stone-400 mb-1 leading-tight line-clamp-2">{label}</p>
                          <p className="text-[20px] font-black leading-none" style={{ color: "#DC2626" }}>{score}</p>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className="space-y-2">
                    {topBaumann.map(({ type, count, pct }) => (
                      <div key={type} className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-stone-400 w-8 shrink-0 text-right">{pct}%</span>
                      </div>
                    ))}
                  </div>
                </CardContent>
          ...
          <div className="px-5 space-y-2">
            {allArticles.map((article) => (
              ...
              >
                <Card className="border-none overflow-hidden" style={{ boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
                  <CardContent className="p-0 flex items-stretch">
                    ...
                    <div className="flex-1 px-3 py-2.5 flex flex-col justify-between min-w-0">
                      <div>
                        <span className="inline-block text-[10px] font-bold px-1.5 py-0.5 rounded-full mb-0.5"
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span className="text-[10px] text-stone-400 truncate min-w-0 mr-2">{article.authorRole}</span>
