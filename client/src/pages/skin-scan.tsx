...
function ResultScreen({ surveyData, analysisResult, imageSrc, faceCroppedSrc, imageBase64, onBack, onGoMagazine, onOpenDiary, user, deferredPrompt, onShowInstallGuide }: any) {
  const { t } = useTranslation();
  const [history, setHistory] = useState<any[]>([]);
    ...
    <AnimatePresence>
      {showPwaPopup && (
        <motion.div
          ...
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        >
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => { setShowPwaPopup(false); localStorage.setItem("fonday_pwa_dismissed", "1"); }} />
          <motion.div
          ...
          >
            ...
            <button
              onClick={() => { setShowPwaPopup(false); localStorage.setItem("fonday_pwa_dismissed", "1"); }}
      ...
      <motion.div className="px-5 pt-6 pb-40 space-y-6" variants={stagger} initial="initial" animate="animate">
        ...
        <Card className="overflow-hidden border-none shadow-2xl rounded-3xl"
          ...
          <CardContent className="p-3.5">
            ...
            <div className="grid grid-cols-[1.15fr_0.85fr_0.95fr] gap-2 mt-3">
              <div className="rounded-2xl px-3 py-4 flex items-end justify-between gap-3" style={{ background: TINT_WARM }}>
                <div className="min-w-0">
                  <p className="text-[9px] font-bold uppercase tracking-[0.12em] text-stone-400">{t("result.overall")}</p>
                  <p className="text-[42px] font-black leading-none mt-1.5" style={{ color: SCAN_TO }}>{overallScore}</p>
                </div>
                <div className="rounded-2xl px-2.5 py-2 text-right shrink-0" style={{ background: "#FFFFFF99" }}>
                  <p className="text-[9px] font-bold text-stone-400">{t("result.scores")}</p>
                  <p className="text-[11px] font-bold mt-1" style={{ color: DEEP_GREEN }}>{weakestSummary || t("result.actionCard.phaseRecord")}</p>
                </div>
              </div>
              <div className="rounded-2xl px-3 py-4 flex flex-col justify-center" style={{ background: "#F7F3FF", border: "1px solid #E9DDFF" }}>
              ...
              <div className="rounded-2xl px-3 py-4 flex flex-col justify-center" style={{ background: "#FFF8EE", border: "1px solid #F4E2C4" }}>
          ...
          <motion.div className="fixed inset-0 z-[110] flex items-end justify-center"
            ...
            <motion.div className="relative bg-white rounded-t-[32px] w-full max-w-md p-6 pb-10 shadow-2xl"
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-base font-bold" style={{ color: DEEP_GREEN }}>{t("cosmetics.loginGateTitle")}</p>
            ...
            >
              ...
              <div className="flex items-center justify-between gap-3 mb-4">
                <div>
                  ...
                  <p className="text-base font-bold mt-1" style={{ color: DEEP_GREEN }}>{t("result.actionCard.questTitle", { done: questDoneCount, total: questBoard.length })}</p>
                </div>
                <div className="flex items-center gap-2">
